...
export async function updateBookingStatus(req, res) {
    try {
        ...
        try {
            ...
            if (status === "APPROVED") {
                // notify lecturer (include target batch if present)
...
export async function getBookingHistory(req, res) {
    try {
        ...
        // HODs should only see their own bookings (personal hall bookings)
...
export async function createRangeBooking(req, res) {
    try {
        ...
        // normalize date same as seed (local date -> ISO date)
        const dp = date.split('-');
